...
static GtkWidget *create_and_show_message_widget(GtkBuilder *builder, char *message_text, time_t datetime, GtkWidget **message_label, t_chat_data *chat_data, id_t message_id) {
...
static void load_messages(GtkBuilder *builder, t_address *server_address, id_t chat_id, list_t **messages_in_chat, t_chat_data *chat_data) {
    ...
    *messages_in_chat = new_messages_in_chat;
    ...
    for (list_node_t *i = (*messages_in_chat)->head; i != NULL; i = i->next) {
        ...
        message->widget = create_and_show_message_widget(builder, message->data, message->creation_date, &message->label_widget, chat_data, message->message_id);
    }
}
...
void gui_update_messages_list(GtkBuilder *builder, list_t *messages_list, list_t *message_updates_list, char *sended_message, t_chat_data *chat_data) {
    for (list_node_t *i = message_updates_list->head; i != NULL; i = i->next) {
        ...
        if (i->next == NULL && sended_message != NULL) {
            message_update->message.widget = create_and_show_message_widget(builder, sended_message, message_update->message.creation_date, &message_update->message.label_widget, chat_data, message_update->message.message_id);
            free(message_update->message.data);
            message_update->message.data = strdup(sended_message);
        }
        else {
            message_update->message.widget = create_and_show_message_widget(builder, message_update->message.data, message_update->message.creation_date, &message_update->message.label_widget, chat_data, message_update->message.message_id);
        }
...
static void gui_send_message_and_update_messages_list(GtkBuilder *builder, t_address *server_address, id_t user_id, id_t chat_id, char *message, list_t *messages_in_chat, t_chat_data *chat_data) {
    ...
    gui_update_messages_list(builder, messages_in_chat, message_updates_list, message, chat_data);
    list_destroy(message_updates_list);
    set_entry_text(builder, NEW_MESSAGE_ENTRY_ID, "");
}

static void update_messages_list(GtkBuilder *builder, t_address *server_address, id_t chat_id, list_t *messages_list, t_chat_data *chat_data) {
    list_t *message_updates_list = rq_get_message_updates(*server_address, chat_id, messages_list);
    if (toggle_widget_visibility(!message_updates_list, builder, CONNECTING_BOX_ID)) return;
    gui_update_messages_list(builder, messages_list, message_updates_list, NULL, chat_data);
    list_destroy(message_updates_list);
}

static gboolean on_update_tick(gpointer user_data) {
    t_chat_data *chat_data = (t_chat_data *)user_data;
    update_messages_list(chat_data->gui_data.builder, &chat_data->gui_data.server_address, chat_data->chat.id, chat_data->messages, chat_data);
    return TRUE;
}
...
static void gui_open_chat(t_chat_data *chat_data) {
    ...
    load_messages(chat_data->gui_data.builder, &chat_data->gui_data.server_address, chat_data->chat.id, &chat_data->messages, chat_data);
...
void on_send_message_clicked(GtkEntry *entry, gpointer *user_data) {
    ...
    gui_send_message_and_update_messages_list(chat_data->gui_data.builder, &chat_data->gui_data.server_address, chat_data->gui_data.user_id, chat_data->chat.id, message_text, chat_data->messages, chat_data);
}
