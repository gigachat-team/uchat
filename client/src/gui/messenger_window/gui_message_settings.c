...
void on_message_delete_clicked(GtkButton *b, gpointer user_data) {
    ...
    list_t *message_updates_list = rq_delete_message_and_get_message_updates(ServerAddress, data->message_id, data->chat_data->chat.id, data->chat_data->messages);
...
void on_change_message(GtkEntry *entry, gpointer *user_data) {
    ...
    list_t *message_updates_list = rq_change_message_and_get_message_updates(ServerAddress, data->message_id, message_text, data->chat_data->chat.id, data->chat_data->messages);
    if (!message_updates_list) return;
