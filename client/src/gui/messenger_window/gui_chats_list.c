...
static void create_button_in_chat_list(id_t user_id, t_chat *chat) {
    GtkWidget *new_button = gtk_button_new_with_label(chat->name);
    t_chat_data *chat_data = create_chat_data_ptr(user_id, chat->name, chat->id, chat->owner_id);
...
void gui_render_chats_list(id_t user_id) {
    list_t *chats_list = rq_get_chats_i_am_in(ServerAddress, user_id);
    ...
    if (chats_list->len != 0) {
        for (list_node_t *i = chats_list->head; i != NULL; i = i->next) {
            create_button_in_chat_list(user_id, i->val);
        }
    }
    else {
...
static void gui_create_chat(id_t user_id) {
    char *chat_name = get_entry_text(Builder, NEW_CHAT_NAME_ENTRY_ID);
    id_t created_chat_id = rq_create_chat(ServerAddress, chat_name, user_id);
...
void on_create_chat_button_clicked(GtkButton *b, gpointer user_data) {
    ...
    gui_create_chat(gui_data->user_id);
    (void)b;
}
